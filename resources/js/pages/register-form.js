...
import { required, maxLen, minLen, emailFormat, digitsExactly, digitsOnly } from '../core/validate.js';
...
export function registerRegisterForm(Alpine) {
    ...
    /* The account form on an invitation link (InvitationResponseController::register): the
     * email comes from the invitation, so only the person's own details are asked for. */
    Alpine.data('invitationRegisterForm', () => ({
        handleSubmit(event) {
            runClientValidation(event,
