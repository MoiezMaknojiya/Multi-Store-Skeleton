 ...
 * shared CRUD table (fetch/search/pagination), plus the super-admin yearly
 * storage panel (partition status + one-click maintenance).
 */
...
export function registerActivityTable(Alpine) {
    Alpine.data('activityTable', (config = {}) => createCrudTable({
        ...
        extraMethods: {
            onInit() {
                if (this.isSuperAdmin) this.fetchPartitions();
            },
